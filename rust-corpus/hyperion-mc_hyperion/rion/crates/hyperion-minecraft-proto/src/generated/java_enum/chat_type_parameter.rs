...
//! `net.minecraft.network.chat.ChatTypeDecoration$Parameter`, as a closed Rust enum.

use crate::{Decode, Encode};

/// `net.minecraft.network.chat.ChatTypeDecoration$Parameter` (3 constants).
///
/// The number on the wire is `ordinal()`, which is what `FriendlyByteBuf.writeEnum` sends. The discriminant *is* that number, so
/// [`ChatTypeParameter::id`] is an `as` cast the optimiser deletes.
///
...
/// be in the set is a number arriving off the wire, and [`ChatTypeParameter::from_id`] is
...
/// declaration in `net.minecraft.network.chat.ChatTypeDecoration$Parameter`.
pub static NAMES_IN_ID_ORDER: [&str; 3] = ["SENDER", "TARGET", "CONTENT"];

/// Discriminants ordered by name, so [`ChatTypeParameter::from_name`] is a binary search
/// and not a scan. Sorted at generation time rather than assumed of the class.
static BY_NAME: [u8; 3] = [2, 0, 1];

impl ChatTypeParameter {
    /// Every constant, in wire-number order, so the index is the number.
    pub const ALL: &'static [Self] = &[Self::Sender, Self::Target, Self::Content];
    /// How many constants this version declares, so every valid number is
    /// below it.
    pub const COUNT: usize = 3;
    /// The Java class these values come from.
    pub const JAVA_CLASS: &'static str = "net.minecraft.network.chat.ChatTypeDecoration$Parameter";
    ...
    /// Mojang's name for the constant, e.g. `SENDER`.
    ///
    ...
    ///
    /// The decode boundary, and the only place a `ChatTypeParameter` can fail to exist.
    #[must_use]
...
impl core::fmt::Display for ChatTypeParameter {
...
const _: () = assert!(ChatTypeParameter::ALL[2].id() == 2);
...
const _: () = assert!(core::mem::size_of::<ChatTypeParameter>() == 1);
