...
//! `net.minecraft.world.scores.Team$Visibility`, as a closed Rust enum.

use crate::{Decode, Encode};

/// `net.minecraft.world.scores.Team$Visibility` (4 constants).
///
...
/// [`TeamVisibility::id`] is an `as` cast the optimiser deletes.
///
...
/// be in the set is a number arriving off the wire, and [`TeamVisibility::from_id`] is
...
/// declaration in `net.minecraft.world.scores.Team$Visibility`.
...
/// Discriminants ordered by name, so [`TeamVisibility::from_name`] is a binary search
/// and not a scan. Sorted at generation time rather than assumed of the class.
static BY_NAME: [u8; 4] = [0, 2, 3, 1];
...
impl TeamVisibility {
    ...
    pub const JAVA_CLASS: &'static str = "net.minecraft.world.scores.Team$Visibility";
    ...
    ///
    /// The decode boundary, and the only place a `TeamVisibility` can fail to exist.
    #[must_use]
...
impl core::fmt::Display for TeamVisibility {
...
const _: () = assert!(TeamVisibility::ALL[3].id() == 3);
...
const _: () = assert!(core::mem::size_of::<TeamVisibility>() == 1);
