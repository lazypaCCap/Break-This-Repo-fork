...
//! `net.minecraft.world.level.levelgen.Heightmap$Types`, as a closed Rust enum.

use crate::{Decode, Encode};

/// `net.minecraft.world.level.levelgen.Heightmap$Types` (6 constants).
///
...
/// [`HeightmapKind::id`] is an `as` cast the optimiser deletes.
///
...
/// be in the set is a number arriving off the wire, and [`HeightmapKind::from_id`] is
...
/// declaration in `net.minecraft.world.level.levelgen.Heightmap$Types`.
...
/// Discriminants ordered by name, so [`HeightmapKind::from_name`] is a binary search
/// and not a scan. Sorted at generation time rather than assumed of the class.
static BY_NAME: [u8; 6] = [4, 5, 3, 2, 1, 0];
...
impl HeightmapKind {
    ...
    pub const JAVA_CLASS: &'static str = "net.minecraft.world.level.levelgen.Heightmap$Types";
    ...
    /// Mojang's name for the constant, e.g. `WORLD_SURFACE_WG`.
    ///
    ...
    ///
    /// The decode boundary, and the only place a `HeightmapKind` can fail to exist.
    #[must_use]
...
impl core::fmt::Display for HeightmapKind {
...
const _: () = assert!(HeightmapKind::ALL[5].id() == 5);
...
const _: () = assert!(core::mem::size_of::<HeightmapKind>() == 1);
