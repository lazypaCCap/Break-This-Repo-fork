...
//! `net.minecraft.world.scores.criteria.ObjectiveCriteria$RenderType`, as a closed Rust enum.

use crate::{Decode, Encode};

/// `net.minecraft.world.scores.criteria.ObjectiveCriteria$RenderType` (2 constants).
///
/// The number on the wire is `ordinal()`, which is what `FriendlyByteBuf.writeEnum` sends. The discriminant *is* that number, so
/// [`ObjectiveRenderType::id`] is an `as` cast the optimiser deletes.
///
...
/// be in the set is a number arriving off the wire, and [`ObjectiveRenderType::from_id`] is
...
/// declaration in `net.minecraft.world.scores.criteria.ObjectiveCriteria$RenderType`.
pub static NAMES_IN_ID_ORDER: [&str; 2] = ["INTEGER", "HEARTS"];

/// Discriminants ordered by name, so [`ObjectiveRenderType::from_name`] is a binary search
...
impl ObjectiveRenderType {
    ...
    /// Mojang's name for the constant, e.g. `INTEGER`.
    ///
    ...
    ///
    /// The decode boundary, and the only place a `ObjectiveRenderType` can fail to exist.
    #[must_use]
...
impl core::fmt::Display for ObjectiveRenderType {
...
const _: () = assert!(ObjectiveRenderType::ALL[1].id() == 1);
...
const _: () = assert!(core::mem::size_of::<ObjectiveRenderType>() == 1);
