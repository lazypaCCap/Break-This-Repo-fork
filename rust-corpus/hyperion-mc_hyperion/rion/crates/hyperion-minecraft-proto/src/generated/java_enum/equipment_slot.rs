...
//! `net.minecraft.world.entity.EquipmentSlot`, as a closed Rust enum.

use crate::{Decode, Encode};

/// `net.minecraft.world.entity.EquipmentSlot` (8 constants).
///
/// The number on the wire is `ordinal()`, which is what `FriendlyByteBuf.writeEnum` sends. The discriminant *is* that number, so
/// [`EquipmentSlot::id`] is an `as` cast the optimiser deletes.
///
...
/// be in the set is a number arriving off the wire, and [`EquipmentSlot::from_id`] is
...
/// declaration in `net.minecraft.world.entity.EquipmentSlot`.
pub static NAMES_IN_ID_ORDER: [&str; 8] = [
    "MAINHAND", "OFFHAND", "FEET", "LEGS", "CHEST", "HEAD", "BODY", "SADDLE",
];

/// Discriminants ordered by name, so [`EquipmentSlot::from_name`] is a binary search
/// and not a scan. Sorted at generation time rather than assumed of the class.
static BY_NAME: [u8; 8] = [6, 4, 2, 5, 3, 0, 1, 7];
...
impl EquipmentSlot {
    ...
    pub const JAVA_CLASS: &'static str = "net.minecraft.world.entity.EquipmentSlot";
    ...
    /// Mojang's name for the constant, e.g. `MAINHAND`.
    ///
    ...
    ///
    /// The decode boundary, and the only place a `EquipmentSlot` can fail to exist.
    #[must_use]
...
impl core::fmt::Display for EquipmentSlot {
...
const _: () = assert!(EquipmentSlot::ALL[7].id() == 7);
...
const _: () = assert!(core::mem::size_of::<EquipmentSlot>() == 1);
