...
//! `minecraft:dialog_action_type`, as a closed Rust enum.

use crate::types::RegistryId;

/// An entry of `minecraft:dialog_action_type` (9 of them).
///
...
/// is an id arriving off the wire, and [`DialogActionType::from_id`] is the only door for
/// it.
///
/// The discriminant is the network id, so [`DialogActionType::id`] is an `as` cast: a
/// known variant's id folds to a constant at compile time.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, PartialOrd, Ord)]
#[repr(u8)]
pub enum DialogActionType {
    /// `minecraft:open_url` (id 0).
    OpenUrl = 0,
    /// `minecraft:run_command` (id 1).
    RunCommand = 1,
    /// `minecraft:suggest_command` (id 2).
    SuggestCommand = 2,
    /// `minecraft:show_dialog` (id 3).
    ShowDialog = 3,
    /// `minecraft:change_page` (id 4).
    ChangePage = 4,
    /// `minecraft:copy_to_clipboard` (id 5).
    CopyToClipboard = 5,
    /// `minecraft:custom` (id 6).
    Custom = 6,
    /// `minecraft:dynamic/run_command` (id 7).
    DynamicRunCommand = 7,
    /// `minecraft:dynamic/custom` (id 8).
    DynamicCustom = 8,
}

/// Entry names in network-id order, so the index is the id.
///
/// A table and not a `match`, so [`DialogActionType::name`] is one load from a static.
/// `crate::generated::registry::DIALOG_ACTION_TYPE` points at this same array, which is
...
/// Discriminants ordered by name, so [`DialogActionType::from_name`] is a binary search
/// and not a scan.
///
...
static BY_NAME: [u8; 9] = [4, 5, 6, 8, 7, 0, 1, 3, 2];
...
impl DialogActionType {
    ...
    /// The namespaced name, e.g. `minecraft:open_url`.
    ///
    ...
    ///
    /// The decode boundary, and the only place a `DialogActionType` can fail to exist.
...
impl core::fmt::Display for DialogActionType {
...
const _: () = assert!(DialogActionType::ALL[8].id().0 == 8);
...
// which is 16.
const _: () = assert!(core::mem::size_of::<DialogActionType>() == 1);
