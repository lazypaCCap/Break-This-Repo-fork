...
//! `minecraft:enchantment_level_based_value_type`, as a closed Rust enum.

use crate::types::RegistryId;

/// An entry of `minecraft:enchantment_level_based_value_type` (6 of them).
///
...
/// is an id arriving off the wire, and [`EnchantmentLevelBasedValueType::from_id`] is the only door for
/// it.
///
/// The discriminant is the network id, so [`EnchantmentLevelBasedValueType::id`] is an `as` cast: a
/// known variant's id folds to a constant at compile time.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, PartialOrd, Ord)]
#[repr(u8)]
pub enum EnchantmentLevelBasedValueType {
    /// `minecraft:clamped` (id 0).
    Clamped = 0,
    /// `minecraft:fraction` (id 1).
    Fraction = 1,
    /// `minecraft:levels_squared` (id 2).
    LevelsSquared = 2,
    /// `minecraft:linear` (id 3).
    Linear = 3,
    /// `minecraft:exponent` (id 4).
    Exponent = 4,
    /// `minecraft:lookup` (id 5).
    Lookup = 5,
}

/// Entry names in network-id order, so the index is the id.
///
/// A table and not a `match`, so [`EnchantmentLevelBasedValueType::name`] is one load from a static.
/// `crate::generated::registry::ENCHANTMENT_LEVEL_BASED_VALUE_TYPE` points at this same array, which is
...
/// Discriminants ordered by name, so [`EnchantmentLevelBasedValueType::from_name`] is a binary search
/// and not a scan.
///
...
static BY_NAME: [u8; 6] = [0, 4, 1, 2, 3, 5];
...
impl EnchantmentLevelBasedValueType {
    ...
    pub const REGISTRY: &'static str = "minecraft:enchantment_level_based_value_type";
    ...
    /// The namespaced name, e.g. `minecraft:clamped`.
    ///
    ...
    ///
    /// The decode boundary, and the only place a `EnchantmentLevelBasedValueType` can fail to exist.
...
impl core::fmt::Display for EnchantmentLevelBasedValueType {
...
const _: () = assert!(EnchantmentLevelBasedValueType::ALL[5].id().0 == 5);
...
// which is 16.
const _: () = assert!(core::mem::size_of::<EnchantmentLevelBasedValueType>() == 1);
