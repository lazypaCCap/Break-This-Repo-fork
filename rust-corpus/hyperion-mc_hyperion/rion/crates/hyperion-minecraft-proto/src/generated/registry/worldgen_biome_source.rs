...
//! `minecraft:worldgen/biome_source`, as a closed Rust enum.

use crate::types::RegistryId;

/// An entry of `minecraft:worldgen/biome_source` (4 of them).
///
...
/// is an id arriving off the wire, and [`WorldgenBiomeSource::from_id`] is the only door for
/// it.
///
/// The discriminant is the network id, so [`WorldgenBiomeSource::id`] is an `as` cast: a
...
pub enum WorldgenBiomeSource {
    ...
    Fixed = 0,
    /// `minecraft:multi_noise` (id 1).
    MultiNoise = 1,
    /// `minecraft:checkerboard` (id 2).
    Checkerboard = 2,
    /// `minecraft:the_end` (id 3).
    TheEnd = 3,
}

/// Entry names in network-id order, so the index is the id.
///
/// A table and not a `match`, so [`WorldgenBiomeSource::name`] is one load from a static.
/// `crate::generated::registry::WORLDGEN_BIOME_SOURCE` points at this same array, which is
...
/// Discriminants ordered by name, so [`WorldgenBiomeSource::from_name`] is a binary search
/// and not a scan.
///
...
impl WorldgenBiomeSource {
    ...
    pub const REGISTRY: &'static str = "minecraft:worldgen/biome_source";
    ...
    ///
    /// The decode boundary, and the only place a `WorldgenBiomeSource` can fail to exist.
...
impl core::fmt::Display for WorldgenBiomeSource {
...
const _: () = assert!(WorldgenBiomeSource::ALL[3].id().0 == 3);
...
// which is 16.
const _: () = assert!(core::mem::size_of::<WorldgenBiomeSource>() == 1);
