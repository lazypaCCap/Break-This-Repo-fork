...
//! `minecraft:float_provider_type`, as a closed Rust enum.

use crate::types::RegistryId;

/// An entry of `minecraft:float_provider_type` (4 of them).
///
...
/// is an id arriving off the wire, and [`FloatProviderType::from_id`] is the only door for
/// it.
///
/// The discriminant is the network id, so [`FloatProviderType::id`] is an `as` cast: a
...
pub enum FloatProviderType {
    ...
    Uniform = 1,
    /// `minecraft:clamped_normal` (id 2).
    ClampedNormal = 2,
    /// `minecraft:trapezoid` (id 3).
    Trapezoid = 3,
}

/// Entry names in network-id order, so the index is the id.
///
/// A table and not a `match`, so [`FloatProviderType::name`] is one load from a static.
/// `crate::generated::registry::FLOAT_PROVIDER_TYPE` points at this same array, which is
...
/// Discriminants ordered by name, so [`FloatProviderType::from_name`] is a binary search
/// and not a scan.
///
...
impl FloatProviderType {
    ...
    pub const REGISTRY: &'static str = "minecraft:float_provider_type";
    ...
    ///
    /// The decode boundary, and the only place a `FloatProviderType` can fail to exist.
...
impl core::fmt::Display for FloatProviderType {
...
const _: () = assert!(FloatProviderType::ALL[3].id().0 == 3);
...
// which is 16.
const _: () = assert!(core::mem::size_of::<FloatProviderType>() == 1);
