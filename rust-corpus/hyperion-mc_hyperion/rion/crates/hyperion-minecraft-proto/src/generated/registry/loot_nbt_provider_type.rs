...
//! `minecraft:loot_nbt_provider_type`, as a closed Rust enum.

use crate::types::RegistryId;

/// An entry of `minecraft:loot_nbt_provider_type` (2 of them).
///
...
/// is an id arriving off the wire, and [`LootNbtProviderType::from_id`] is the only door for
/// it.
///
/// The discriminant is the network id, so [`LootNbtProviderType::id`] is an `as` cast: a
/// known variant's id folds to a constant at compile time.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, PartialOrd, Ord)]
#[repr(u8)]
pub enum LootNbtProviderType {
    /// `minecraft:storage` (id 0).
    Storage = 0,
...
///
/// A table and not a `match`, so [`LootNbtProviderType::name`] is one load from a static.
/// `crate::generated::registry::LOOT_NBT_PROVIDER_TYPE` points at this same array, which is
/// what stops the two from disagreeing.
pub static NAMES_IN_ID_ORDER: [&str; 2] = ["minecraft:storage", "minecraft:context"];

/// Discriminants ordered by name, so [`LootNbtProviderType::from_name`] is a binary search
/// and not a scan.
///
...
impl LootNbtProviderType {
    ...
    pub const ALL: &'static [Self] = &[Self::Storage, Self::Context];
    /// How many entries this version has, so every valid id is below it.
    pub const COUNT: usize = 2;
    /// The registry these values belong to.
    pub const REGISTRY: &'static str = "minecraft:loot_nbt_provider_type";
    ...
    /// The namespaced name, e.g. `minecraft:storage`.
    ///
    ...
    ///
    /// The decode boundary, and the only place a `LootNbtProviderType` can fail to exist.
...
impl core::fmt::Display for LootNbtProviderType {
...
const _: () = assert!(LootNbtProviderType::ALL[1].id().0 == 1);
...
// which is 16.
const _: () = assert!(core::mem::size_of::<LootNbtProviderType>() == 1);
