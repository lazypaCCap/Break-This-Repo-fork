...
//! `minecraft:loot_score_provider_type`, as a closed Rust enum.

use crate::types::RegistryId;

/// An entry of `minecraft:loot_score_provider_type` (2 of them).
///
...
/// is an id arriving off the wire, and [`LootScoreProviderType::from_id`] is the only door for
/// it.
///
/// The discriminant is the network id, so [`LootScoreProviderType::id`] is an `as` cast: a
/// known variant's id folds to a constant at compile time.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, PartialOrd, Ord)]
#[repr(u8)]
pub enum LootScoreProviderType {
    /// `minecraft:fixed` (id 0).
    Fixed = 0,
    /// `minecraft:context` (id 1).
    Context = 1,
}

/// Entry names in network-id order, so the index is the id.
///
/// A table and not a `match`, so [`LootScoreProviderType::name`] is one load from a static.
/// `crate::generated::registry::LOOT_SCORE_PROVIDER_TYPE` points at this same array, which is
/// what stops the two from disagreeing.
pub static NAMES_IN_ID_ORDER: [&str; 2] = ["minecraft:fixed", "minecraft:context"];

/// Discriminants ordered by name, so [`LootScoreProviderType::from_name`] is a binary search
/// and not a scan.
///
...
impl LootScoreProviderType {
    ...
    pub const ALL: &'static [Self] = &[Self::Fixed, Self::Context];
    /// How many entries this version has, so every valid id is below it.
    pub const COUNT: usize = 2;
    /// The registry these values belong to.
    pub const REGISTRY: &'static str = "minecraft:loot_score_provider_type";
    ...
    /// The namespaced name, e.g. `minecraft:fixed`.
    ///
    ...
    ///
    /// The decode boundary, and the only place a `LootScoreProviderType` can fail to exist.
...
impl core::fmt::Display for LootScoreProviderType {
...
const _: () = assert!(LootScoreProviderType::ALL[1].id().0 == 1);
...
// which is 16.
const _: () = assert!(core::mem::size_of::<LootScoreProviderType>() == 1);
