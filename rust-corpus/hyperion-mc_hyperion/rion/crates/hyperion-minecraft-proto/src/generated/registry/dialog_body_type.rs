...
//! `minecraft:dialog_body_type`, as a closed Rust enum.

use crate::types::RegistryId;

/// An entry of `minecraft:dialog_body_type` (2 of them).
///
...
/// is an id arriving off the wire, and [`DialogBodyType::from_id`] is the only door for
/// it.
///
/// The discriminant is the network id, so [`DialogBodyType::id`] is an `as` cast: a
...
pub enum DialogBodyType {
    /// `minecraft:item` (id 0).
    Item = 0,
    /// `minecraft:plain_message` (id 1).
    PlainMessage = 1,
}

/// Entry names in network-id order, so the index is the id.
///
/// A table and not a `match`, so [`DialogBodyType::name`] is one load from a static.
/// `crate::generated::registry::DIALOG_BODY_TYPE` points at this same array, which is
/// what stops the two from disagreeing.
pub static NAMES_IN_ID_ORDER: [&str; 2] = ["minecraft:item", "minecraft:plain_message"];

/// Discriminants ordered by name, so [`DialogBodyType::from_name`] is a binary search
/// and not a scan.
///
...
impl DialogBodyType {
    ...
    pub const ALL: &'static [Self] = &[Self::Item, Self::PlainMessage];
    /// How many entries this version has, so every valid id is below it.
    pub const COUNT: usize = 2;
    /// The registry these values belong to.
    pub const REGISTRY: &'static str = "minecraft:dialog_body_type";
    ...
    /// The namespaced name, e.g. `minecraft:item`.
    ///
    ...
    ///
    /// The decode boundary, and the only place a `DialogBodyType` can fail to exist.
...
impl core::fmt::Display for DialogBodyType {
...
const _: () = assert!(DialogBodyType::ALL[1].id().0 == 1);
...
// which is 16.
const _: () = assert!(core::mem::size_of::<DialogBodyType>() == 1);
