...
//! `minecraft:recipe_display`, as a closed Rust enum.

use crate::types::RegistryId;

/// An entry of `minecraft:recipe_display` (5 of them).
///
...
/// is an id arriving off the wire, and [`RecipeDisplay::from_id`] is the only door for
/// it.
///
/// The discriminant is the network id, so [`RecipeDisplay::id`] is an `as` cast: a
/// known variant's id folds to a constant at compile time.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, PartialOrd, Ord)]
#[repr(u8)]
pub enum RecipeDisplay {
    /// `minecraft:crafting_shapeless` (id 0).
    CraftingShapeless = 0,
    /// `minecraft:crafting_shaped` (id 1).
    CraftingShaped = 1,
    /// `minecraft:furnace` (id 2).
    Furnace = 2,
    /// `minecraft:stonecutter` (id 3).
    Stonecutter = 3,
    /// `minecraft:smithing` (id 4).
    Smithing = 4,
}

/// Entry names in network-id order, so the index is the id.
///
/// A table and not a `match`, so [`RecipeDisplay::name`] is one load from a static.
/// `crate::generated::registry::RECIPE_DISPLAY` points at this same array, which is
...
/// Discriminants ordered by name, so [`RecipeDisplay::from_name`] is a binary search
/// and not a scan.
///
...
impl RecipeDisplay {
    ...
    pub const REGISTRY: &'static str = "minecraft:recipe_display";
    ...
    /// The namespaced name, e.g. `minecraft:crafting_shapeless`.
    ///
    ...
    ///
    /// The decode boundary, and the only place a `RecipeDisplay` can fail to exist.
...
impl core::fmt::Display for RecipeDisplay {
...
const _: () = assert!(RecipeDisplay::ALL[4].id().0 == 4);
...
// which is 16.
const _: () = assert!(core::mem::size_of::<RecipeDisplay>() == 1);
