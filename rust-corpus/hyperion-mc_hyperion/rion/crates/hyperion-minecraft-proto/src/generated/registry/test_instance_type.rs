...
//! `minecraft:test_instance_type`, as a closed Rust enum.

use crate::types::RegistryId;

/// An entry of `minecraft:test_instance_type` (2 of them).
///
...
/// is an id arriving off the wire, and [`TestInstanceType::from_id`] is the only door for
/// it.
///
/// The discriminant is the network id, so [`TestInstanceType::id`] is an `as` cast: a
/// known variant's id folds to a constant at compile time.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, PartialOrd, Ord)]
#[repr(u8)]
pub enum TestInstanceType {
    /// `minecraft:block_based` (id 0).
    BlockBased = 0,
    /// `minecraft:function` (id 1).
    Function = 1,
}

/// Entry names in network-id order, so the index is the id.
///
/// A table and not a `match`, so [`TestInstanceType::name`] is one load from a static.
/// `crate::generated::registry::TEST_INSTANCE_TYPE` points at this same array, which is
/// what stops the two from disagreeing.
pub static NAMES_IN_ID_ORDER: [&str; 2] = ["minecraft:block_based", "minecraft:function"];

/// Discriminants ordered by name, so [`TestInstanceType::from_name`] is a binary search
/// and not a scan.
///
...
impl TestInstanceType {
    ...
    pub const ALL: &'static [Self] = &[Self::BlockBased, Self::Function];
    /// How many entries this version has, so every valid id is below it.
    pub const COUNT: usize = 2;
    /// The registry these values belong to.
    pub const REGISTRY: &'static str = "minecraft:test_instance_type";
    ...
    /// The namespaced name, e.g. `minecraft:block_based`.
    ///
    ...
    ///
    /// The decode boundary, and the only place a `TestInstanceType` can fail to exist.
...
impl core::fmt::Display for TestInstanceType {
...
const _: () = assert!(TestInstanceType::ALL[1].id().0 == 1);
...
// which is 16.
const _: () = assert!(core::mem::size_of::<TestInstanceType>() == 1);
