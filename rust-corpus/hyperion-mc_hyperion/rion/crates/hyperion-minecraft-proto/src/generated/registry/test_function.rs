...
//! `minecraft:test_function`, as a closed Rust enum.

use crate::types::RegistryId;

/// An entry of `minecraft:test_function` (1 of them).
///
...
/// is an id arriving off the wire, and [`TestFunction::from_id`] is the only door for
/// it.
///
/// The discriminant is the network id, so [`TestFunction::id`] is an `as` cast: a
...
///
/// A table and not a `match`, so [`TestFunction::name`] is one load from a static.
/// `crate::generated::registry::TEST_FUNCTION` points at this same array, which is
/// what stops the two from disagreeing.
pub static NAMES_IN_ID_ORDER: [&str; 1] = ["minecraft:always_pass"];

/// Discriminants ordered by name, so [`TestFunction::from_name`] is a binary search
/// and not a scan.
///
...
impl TestFunction {
    ...
    ///
    /// The decode boundary, and the only place a `TestFunction` can fail to exist.
...
const _: () = assert!(TestFunction::ALL[0].id().0 == 0);
...
// which is 16.
const _: () = assert!(core::mem::size_of::<TestFunction>() == 1);
