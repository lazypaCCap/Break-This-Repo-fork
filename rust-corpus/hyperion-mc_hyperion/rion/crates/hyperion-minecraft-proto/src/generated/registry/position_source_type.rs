...
//! `minecraft:position_source_type`, as a closed Rust enum.

use crate::types::RegistryId;

/// An entry of `minecraft:position_source_type` (2 of them).
///
...
/// is an id arriving off the wire, and [`PositionSourceType::from_id`] is the only door for
/// it.
///
/// The discriminant is the network id, so [`PositionSourceType::id`] is an `as` cast: a
...
pub enum PositionSourceType {
    ...
    Block = 0,
    /// `minecraft:entity` (id 1).
    Entity = 1,
}

/// Entry names in network-id order, so the index is the id.
///
/// A table and not a `match`, so [`PositionSourceType::name`] is one load from a static.
/// `crate::generated::registry::POSITION_SOURCE_TYPE` points at this same array, which is
/// what stops the two from disagreeing.
pub static NAMES_IN_ID_ORDER: [&str; 2] = ["minecraft:block", "minecraft:entity"];

/// Discriminants ordered by name, so [`PositionSourceType::from_name`] is a binary search
/// and not a scan.
///
...
impl PositionSourceType {
    ...
    pub const ALL: &'static [Self] = &[Self::Block, Self::Entity];
    /// How many entries this version has, so every valid id is below it.
    pub const COUNT: usize = 2;
    /// The registry these values belong to.
    pub const REGISTRY: &'static str = "minecraft:position_source_type";
    ...
    ///
    /// The decode boundary, and the only place a `PositionSourceType` can fail to exist.
...
impl core::fmt::Display for PositionSourceType {
...
const _: () = assert!(PositionSourceType::ALL[1].id().0 == 1);
...
// which is 16.
const _: () = assert!(core::mem::size_of::<PositionSourceType>() == 1);
