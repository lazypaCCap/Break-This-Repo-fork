...
//! `minecraft:worldgen/pool_alias_binding`, as a closed Rust enum.

use crate::types::RegistryId;

/// An entry of `minecraft:worldgen/pool_alias_binding` (3 of them).
///
...
/// is an id arriving off the wire, and [`WorldgenPoolAliasBinding::from_id`] is the only door for
/// it.
///
/// The discriminant is the network id, so [`WorldgenPoolAliasBinding::id`] is an `as` cast: a
/// known variant's id folds to a constant at compile time.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, PartialOrd, Ord)]
#[repr(u8)]
pub enum WorldgenPoolAliasBinding {
    /// `minecraft:random` (id 0).
    Random = 0,
    /// `minecraft:random_group` (id 1).
    RandomGroup = 1,
    /// `minecraft:direct` (id 2).
    Direct = 2,
}

/// Entry names in network-id order, so the index is the id.
///
/// A table and not a `match`, so [`WorldgenPoolAliasBinding::name`] is one load from a static.
/// `crate::generated::registry::WORLDGEN_POOL_ALIAS_BINDING` points at this same array, which is
...
/// Discriminants ordered by name, so [`WorldgenPoolAliasBinding::from_name`] is a binary search
/// and not a scan.
///
...
impl WorldgenPoolAliasBinding {
    ...
    pub const ALL: &'static [Self] = &[Self::Random, Self::RandomGroup, Self::Direct];
    /// How many entries this version has, so every valid id is below it.
    pub const COUNT: usize = 3;
    /// The registry these values belong to.
    pub const REGISTRY: &'static str = "minecraft:worldgen/pool_alias_binding";
    ...
    /// The namespaced name, e.g. `minecraft:random`.
    ///
    ...
    ///
    /// The decode boundary, and the only place a `WorldgenPoolAliasBinding` can fail to exist.
...
impl core::fmt::Display for WorldgenPoolAliasBinding {
...
const _: () = assert!(WorldgenPoolAliasBinding::ALL[2].id().0 == 2);
...
// which is 16.
const _: () = assert!(core::mem::size_of::<WorldgenPoolAliasBinding>() == 1);
