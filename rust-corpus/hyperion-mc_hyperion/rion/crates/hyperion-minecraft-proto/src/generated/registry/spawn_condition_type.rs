...
//! `minecraft:spawn_condition_type`, as a closed Rust enum.

use crate::types::RegistryId;

/// An entry of `minecraft:spawn_condition_type` (3 of them).
///
...
/// is an id arriving off the wire, and [`SpawnConditionType::from_id`] is the only door for
/// it.
///
/// The discriminant is the network id, so [`SpawnConditionType::id`] is an `as` cast: a
/// known variant's id folds to a constant at compile time.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, PartialOrd, Ord)]
#[repr(u8)]
pub enum SpawnConditionType {
    /// `minecraft:structure` (id 0).
    Structure = 0,
    /// `minecraft:moon_brightness` (id 1).
    MoonBrightness = 1,
    /// `minecraft:biome` (id 2).
    Biome = 2,
}

/// Entry names in network-id order, so the index is the id.
///
/// A table and not a `match`, so [`SpawnConditionType::name`] is one load from a static.
/// `crate::generated::registry::SPAWN_CONDITION_TYPE` points at this same array, which is
...
/// Discriminants ordered by name, so [`SpawnConditionType::from_name`] is a binary search
/// and not a scan.
///
...
impl SpawnConditionType {
    ...
    pub const ALL: &'static [Self] = &[Self::Structure, Self::MoonBrightness, Self::Biome];
    /// How many entries this version has, so every valid id is below it.
    pub const COUNT: usize = 3;
    /// The registry these values belong to.
    pub const REGISTRY: &'static str = "minecraft:spawn_condition_type";
    ...
    /// The namespaced name, e.g. `minecraft:structure`.
    ///
    ...
    ///
    /// The decode boundary, and the only place a `SpawnConditionType` can fail to exist.
...
impl core::fmt::Display for SpawnConditionType {
...
const _: () = assert!(SpawnConditionType::ALL[2].id().0 == 2);
...
// which is 16.
const _: () = assert!(core::mem::size_of::<SpawnConditionType>() == 1);
