...
//! `minecraft:worldgen/chunk_generator`, as a closed Rust enum.

use crate::types::RegistryId;

/// An entry of `minecraft:worldgen/chunk_generator` (3 of them).
///
...
/// is an id arriving off the wire, and [`WorldgenChunkGenerator::from_id`] is the only door for
/// it.
///
/// The discriminant is the network id, so [`WorldgenChunkGenerator::id`] is an `as` cast: a
/// known variant's id folds to a constant at compile time.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, PartialOrd, Ord)]
#[repr(u8)]
pub enum WorldgenChunkGenerator {
    /// `minecraft:noise` (id 0).
    Noise = 0,
    /// `minecraft:flat` (id 1).
    Flat = 1,
    /// `minecraft:debug` (id 2).
    Debug = 2,
}

/// Entry names in network-id order, so the index is the id.
///
/// A table and not a `match`, so [`WorldgenChunkGenerator::name`] is one load from a static.
/// `crate::generated::registry::WORLDGEN_CHUNK_GENERATOR` points at this same array, which is
/// what stops the two from disagreeing.
pub static NAMES_IN_ID_ORDER: [&str; 3] = ["minecraft:noise", "minecraft:flat", "minecraft:debug"];

/// Discriminants ordered by name, so [`WorldgenChunkGenerator::from_name`] is a binary search
/// and not a scan.
///
...
impl WorldgenChunkGenerator {
    ...
    /// The namespaced name, e.g. `minecraft:noise`.
    ///
    ...
    ///
    /// The decode boundary, and the only place a `WorldgenChunkGenerator` can fail to exist.
...
impl core::fmt::Display for WorldgenChunkGenerator {
...
const _: () = assert!(WorldgenChunkGenerator::ALL[2].id().0 == 2);
...
// which is 16.
const _: () = assert!(core::mem::size_of::<WorldgenChunkGenerator>() == 1);
