...
//! `minecraft:test_environment_definition_type`, as a closed Rust enum.

use crate::types::RegistryId;

/// An entry of `minecraft:test_environment_definition_type` (7 of them).
///
...
/// is an id arriving off the wire, and [`TestEnvironmentDefinitionType::from_id`] is the only door for
/// it.
///
/// The discriminant is the network id, so [`TestEnvironmentDefinitionType::id`] is an `as` cast: a
...
pub enum TestEnvironmentDefinitionType {
    ...
    AllOf = 0,
    /// `minecraft:clock_time` (id 1).
    ClockTime = 1,
    /// `minecraft:difficulty` (id 2).
    Difficulty = 2,
    /// `minecraft:function` (id 3).
    Function = 3,
    /// `minecraft:game_rules` (id 4).
    GameRules = 4,
    /// `minecraft:timeline_attributes` (id 5).
    TimelineAttributes = 5,
    /// `minecraft:weather` (id 6).
    Weather = 6,
}

/// Entry names in network-id order, so the index is the id.
///
/// A table and not a `match`, so [`TestEnvironmentDefinitionType::name`] is one load from a static.
/// `crate::generated::registry::TEST_ENVIRONMENT_DEFINITION_TYPE` points at this same array, which is
...
/// Discriminants ordered by name, so [`TestEnvironmentDefinitionType::from_name`] is a binary search
/// and not a scan.
///
...
static BY_NAME: [u8; 7] = [0, 1, 2, 3, 4, 5, 6];
...
impl TestEnvironmentDefinitionType {
    ...
    pub const REGISTRY: &'static str = "minecraft:test_environment_definition_type";
    ...
    ///
    /// The decode boundary, and the only place a `TestEnvironmentDefinitionType` can fail to exist.
...
impl core::fmt::Display for TestEnvironmentDefinitionType {
...
const _: () = assert!(TestEnvironmentDefinitionType::ALL[6].id().0 == 6);
...
// which is 16.
const _: () = assert!(core::mem::size_of::<TestEnvironmentDefinitionType>() == 1);
