...
//! `minecraft:number_format_type`, as a closed Rust enum.

use crate::types::RegistryId;

/// An entry of `minecraft:number_format_type` (3 of them).
///
...
/// is an id arriving off the wire, and [`NumberFormatType::from_id`] is the only door for
/// it.
///
/// The discriminant is the network id, so [`NumberFormatType::id`] is an `as` cast: a
/// known variant's id folds to a constant at compile time.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, PartialOrd, Ord)]
#[repr(u8)]
pub enum NumberFormatType {
    /// `minecraft:blank` (id 0).
    Blank = 0,
    /// `minecraft:styled` (id 1).
    Styled = 1,
    /// `minecraft:fixed` (id 2).
    Fixed = 2,
}

/// Entry names in network-id order, so the index is the id.
///
/// A table and not a `match`, so [`NumberFormatType::name`] is one load from a static.
/// `crate::generated::registry::NUMBER_FORMAT_TYPE` points at this same array, which is
/// what stops the two from disagreeing.
pub static NAMES_IN_ID_ORDER: [&str; 3] =
    ["minecraft:blank", "minecraft:styled", "minecraft:fixed"];

/// Discriminants ordered by name, so [`NumberFormatType::from_name`] is a binary search
/// and not a scan.
///
...
impl NumberFormatType {
    ...
    /// The namespaced name, e.g. `minecraft:blank`.
    ///
    ...
    ///
    /// The decode boundary, and the only place a `NumberFormatType` can fail to exist.
...
impl core::fmt::Display for NumberFormatType {
...
const _: () = assert!(NumberFormatType::ALL[2].id().0 == 2);
...
// which is 16.
const _: () = assert!(core::mem::size_of::<NumberFormatType>() == 1);
