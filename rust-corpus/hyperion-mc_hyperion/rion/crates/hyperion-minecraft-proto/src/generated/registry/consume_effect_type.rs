...
//! `minecraft:consume_effect_type`, as a closed Rust enum.

use crate::types::RegistryId;

/// An entry of `minecraft:consume_effect_type` (5 of them).
///
...
/// is an id arriving off the wire, and [`ConsumeEffectType::from_id`] is the only door for
/// it.
///
/// The discriminant is the network id, so [`ConsumeEffectType::id`] is an `as` cast: a
/// known variant's id folds to a constant at compile time.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, PartialOrd, Ord)]
#[repr(u8)]
pub enum ConsumeEffectType {
    /// `minecraft:apply_effects` (id 0).
    ApplyEffects = 0,
    /// `minecraft:remove_effects` (id 1).
    RemoveEffects = 1,
    /// `minecraft:clear_all_effects` (id 2).
    ClearAllEffects = 2,
    /// `minecraft:teleport_randomly` (id 3).
    TeleportRandomly = 3,
    /// `minecraft:play_sound` (id 4).
    PlaySound = 4,
}

/// Entry names in network-id order, so the index is the id.
///
/// A table and not a `match`, so [`ConsumeEffectType::name`] is one load from a static.
/// `crate::generated::registry::CONSUME_EFFECT_TYPE` points at this same array, which is
...
/// Discriminants ordered by name, so [`ConsumeEffectType::from_name`] is a binary search
/// and not a scan.
///
...
impl ConsumeEffectType {
    ...
    pub const REGISTRY: &'static str = "minecraft:consume_effect_type";
    ...
    /// The namespaced name, e.g. `minecraft:apply_effects`.
    ///
    ...
    ///
    /// The decode boundary, and the only place a `ConsumeEffectType` can fail to exist.
...
impl core::fmt::Display for ConsumeEffectType {
...
const _: () = assert!(ConsumeEffectType::ALL[4].id().0 == 4);
...
// which is 16.
const _: () = assert!(core::mem::size_of::<ConsumeEffectType>() == 1);
