...
//! `minecraft:slot_source_type`, as a closed Rust enum.

use crate::types::RegistryId;

/// An entry of `minecraft:slot_source_type` (6 of them).
///
...
/// is an id arriving off the wire, and [`SlotSourceType::from_id`] is the only door for
/// it.
///
/// The discriminant is the network id, so [`SlotSourceType::id`] is an `as` cast: a
/// known variant's id folds to a constant at compile time.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, PartialOrd, Ord)]
#[repr(u8)]
pub enum SlotSourceType {
    /// `minecraft:group` (id 0).
    Group = 0,
    /// `minecraft:filtered` (id 1).
    Filtered = 1,
    /// `minecraft:limit_slots` (id 2).
    LimitSlots = 2,
    /// `minecraft:slot_range` (id 3).
    SlotRange = 3,
    /// `minecraft:contents` (id 4).
    Contents = 4,
    /// `minecraft:empty` (id 5).
    Empty = 5,
}

/// Entry names in network-id order, so the index is the id.
///
/// A table and not a `match`, so [`SlotSourceType::name`] is one load from a static.
/// `crate::generated::registry::SLOT_SOURCE_TYPE` points at this same array, which is
...
/// Discriminants ordered by name, so [`SlotSourceType::from_name`] is a binary search
/// and not a scan.
///
...
static BY_NAME: [u8; 6] = [4, 5, 1, 0, 2, 3];
...
impl SlotSourceType {
    ...
    pub const REGISTRY: &'static str = "minecraft:slot_source_type";
    ...
    /// The namespaced name, e.g. `minecraft:group`.
    ///
    ...
    ///
    /// The decode boundary, and the only place a `SlotSourceType` can fail to exist.
...
impl core::fmt::Display for SlotSourceType {
...
const _: () = assert!(SlotSourceType::ALL[5].id().0 == 5);
...
// which is 16.
const _: () = assert!(core::mem::size_of::<SlotSourceType>() == 1);
