...
//! `minecraft:pos_rule_test`, as a closed Rust enum.

use crate::types::RegistryId;

/// An entry of `minecraft:pos_rule_test` (3 of them).
///
...
/// is an id arriving off the wire, and [`PosRuleTest::from_id`] is the only door for
/// it.
///
/// The discriminant is the network id, so [`PosRuleTest::id`] is an `as` cast: a
...
pub enum PosRuleTest {
    ...
    AlwaysTrue = 0,
    /// `minecraft:linear_pos` (id 1).
    LinearPos = 1,
    /// `minecraft:axis_aligned_linear_pos` (id 2).
    AxisAlignedLinearPos = 2,
}

/// Entry names in network-id order, so the index is the id.
///
/// A table and not a `match`, so [`PosRuleTest::name`] is one load from a static.
/// `crate::generated::registry::POS_RULE_TEST` points at this same array, which is
...
/// Discriminants ordered by name, so [`PosRuleTest::from_name`] is a binary search
/// and not a scan.
///
...
impl PosRuleTest {
    ...
    pub const REGISTRY: &'static str = "minecraft:pos_rule_test";
    ...
    ///
    /// The decode boundary, and the only place a `PosRuleTest` can fail to exist.
...
impl core::fmt::Display for PosRuleTest {
...
const _: () = assert!(PosRuleTest::ALL[2].id().0 == 2);
...
// which is 16.
const _: () = assert!(core::mem::size_of::<PosRuleTest>() == 1);
