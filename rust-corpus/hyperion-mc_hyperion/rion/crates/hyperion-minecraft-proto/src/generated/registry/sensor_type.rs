// @generated by nix/generate-rust.py from Minecraft 26.2 (protocol 776).
// Do not edit by hand. Regenerate with: nix run .#sync-minecraft-proto

//! `minecraft:sensor_type`, as a closed Rust enum.

use crate::types::RegistryId;

/// An entry of `minecraft:sensor_type` (24 of them).
///
/// Closed on purpose. Every value this version defines is a variant, so a
/// caller cannot name one that does not exist and a `match` never needs an arm
/// for a case that cannot occur. The one place a value might not be in the set
/// is an id arriving off the wire, and [`SensorType::from_id`] is the only door for
/// it.
///
/// The discriminant is the network id, so [`SensorType::id`] is an `as` cast: a
/// known variant's id folds to a constant at compile time.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, PartialOrd, Ord)]
#[repr(u8)]
pub enum SensorType {
    /// `minecraft:dummy` (id 0).
    Dummy = 0,
    /// `minecraft:nearest_items` (id 1).
    NearestItems = 1,
    /// `minecraft:nearest_living_entities` (id 2).
    NearestLivingEntities = 2,
    /// `minecraft:nearest_players` (id 3).
    NearestPlayers = 3,
    /// `minecraft:nearest_bed` (id 4).
    NearestBed = 4,
    /// `minecraft:hurt_by` (id 5).
    HurtBy = 5,
    /// `minecraft:villager_hostiles` (id 6).
    VillagerHostiles = 6,
    /// `minecraft:villager_babies` (id 7).
    VillagerBabies = 7,
    /// `minecraft:secondary_pois` (id 8).
    SecondaryPois = 8,
    /// `minecraft:golem_detected` (id 9).
    GolemDetected = 9,
    /// `minecraft:armadillo_scare_detected` (id 10).
    ArmadilloScareDetected = 10,
    /// `minecraft:piglin_specific_sensor` (id 11).
    PiglinSpecificSensor = 11,
    /// `minecraft:piglin_brute_specific_sensor` (id 12).
    PiglinBruteSpecificSensor = 12,
    /// `minecraft:hoglin_specific_sensor` (id 13).
    HoglinSpecificSensor = 13,
    /// `minecraft:nearest_adult` (id 14).
    NearestAdult = 14,
    /// `minecraft:nearest_adult_any_type` (id 15).
    NearestAdultAnyType = 15,
    /// `minecraft:axolotl_attackables` (id 16).
    AxolotlAttackables = 16,
    /// `minecraft:food_temptations` (id 17).
    FoodTemptations = 17,
    /// `minecraft:frog_temptations` (id 18).
    FrogTemptations = 18,
    /// `minecraft:nautilus_temptations` (id 19).
    NautilusTemptations = 19,
    /// `minecraft:frog_attackables` (id 20).
    FrogAttackables = 20,
    /// `minecraft:is_in_water` (id 21).
    IsInWater = 21,
    /// `minecraft:warden_entity_sensor` (id 22).
    WardenEntitySensor = 22,
    /// `minecraft:breeze_attack_entity_sensor` (id 23).
    BreezeAttackEntitySensor = 23,
}

/// Entry names in network-id order, so the index is the id.
///
/// A table and not a `match`, so [`SensorType::name`] is one load from a static.
/// `crate::generated::registry::SENSOR_TYPE` points at this same array, which is
/// what stops the two from disagreeing.
pub static NAMES_IN_ID_ORDER: [&str; 24] = [
    "minecraft:dummy",
    "minecraft:nearest_items",
    "minecraft:nearest_living_entities",
    "minecraft:nearest_players",
    "minecraft:nearest_bed",
    "minecraft:hurt_by",
    "minecraft:villager_hostiles",
    "minecraft:villager_babies",
    "minecraft:secondary_pois",
    "minecraft:golem_detected",
    "minecraft:armadillo_scare_detected",
    "minecraft:piglin_specific_sensor",
    "minecraft:piglin_brute_specific_sensor",
    "minecraft:hoglin_specific_sensor",
    "minecraft:nearest_adult",
    "minecraft:nearest_adult_any_type",
    "minecraft:axolotl_attackables",
    "minecraft:food_temptations",
    "minecraft:frog_temptations",
    "minecraft:nautilus_temptations",
    "minecraft:frog_attackables",
    "minecraft:is_in_water",
    "minecraft:warden_entity_sensor",
    "minecraft:breeze_attack_entity_sensor",
];

/// Discriminants ordered by name, so [`SensorType::from_name`] is a binary search
/// and not a scan.
///
/// Sorted here, at generation time, rather than assumed of the registry:
/// this registry's own order is not name order. A wrong order would fail as a lookup miss and read as "that
/// entry does not exist", so `tests/registry_enum.rs` puts every one of the
/// 24 names back through `from_name` rather than spot-checking.
static BY_NAME: [u8; 24] = [
    10, 16, 23, 0, 17, 20, 18, 9, 13, 5, 21, 19, 14, 15, 4, 1, 2, 3, 12, 11, 8, 7, 6, 22,
];

impl SensorType {
    /// Every entry, in network-id order, so the index is the id.
    pub const ALL: &'static [Self] = &[
        Self::Dummy,
        Self::NearestItems,
        Self::NearestLivingEntities,
        Self::NearestPlayers,
        Self::NearestBed,
        Self::HurtBy,
        Self::VillagerHostiles,
        Self::VillagerBabies,
        Self::SecondaryPois,
        Self::GolemDetected,
        Self::ArmadilloScareDetected,
        Self::PiglinSpecificSensor,
        Self::PiglinBruteSpecificSensor,
        Self::HoglinSpecificSensor,
        Self::NearestAdult,
        Self::NearestAdultAnyType,
        Self::AxolotlAttackables,
        Self::FoodTemptations,
        Self::FrogTemptations,
        Self::NautilusTemptations,
        Self::FrogAttackables,
        Self::IsInWater,
        Self::WardenEntitySensor,
        Self::BreezeAttackEntitySensor,
    ];
    /// How many entries this version has, so every valid id is below it.
    pub const COUNT: usize = 24;
    /// The registry these values belong to.
    pub const REGISTRY: &'static str = "minecraft:sensor_type";

    /// The network id.
    ///
    /// `#[repr(u8)]` on the enum is what makes this an `as` cast rather
    /// than a lookup: for a variant known at compile time the whole call folds
    /// to the number.
    #[must_use]
    pub const fn id(self) -> RegistryId {
        RegistryId(self as i32)
    }

    /// The namespaced name, e.g. `minecraft:dummy`.
    ///
    /// Not `const`: it reads a static table. The direction that is const is
    /// [`Self::id`], which is the one that reaches the wire.
    #[must_use]
    pub fn name(self) -> &'static str {
        NAMES_IN_ID_ORDER[self as usize]
    }

    /// Resolve an id that arrived off the wire.
    ///
    /// The decode boundary, and the only place a `SensorType` can fail to exist.
    /// Every other path is holding a value that is already in the set.
    #[must_use]
    pub fn from_id(id: RegistryId) -> Option<Self> {
        usize::try_from(id.0)
            .ok()
            .and_then(|index| Self::ALL.get(index).copied())
    }

    /// Resolve a name only known at run time -- one read out of a world file,
    /// or typed into a command.
    ///
    /// A name known at compile time should be the variant instead, so that a
    /// version bump dropping it fails to build at the line that wanted it
    /// rather than returning `None` on whichever tick it was next needed.
    #[must_use]
    pub fn from_name(name: &str) -> Option<Self> {
        let found = BY_NAME
            .binary_search_by(|&index| NAMES_IN_ID_ORDER[index as usize].cmp(name))
            .ok()?;
        Self::ALL.get(BY_NAME[found] as usize).copied()
    }
}

impl core::fmt::Display for SensorType {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        f.write_str(self.name())
    }
}

// The zero-cost claim, checked by the compiler rather than by a test.
//
// A `const` context cannot call anything the compiler is unable to evaluate at
// compile time, so if `id()` were ever a table lookup or a `match` over a
// runtime value instead of a cast off the discriminant, this line would stop
// building. It is the proof, not a restatement of it.
const _: () = assert!(SensorType::ALL[23].id().0 == 23);

// And the size, for the same reason: `#[repr(u8)]` is what makes the
// discriminant the id, and a value of this enum is that many bytes and no
// more. The old hand-written `EntityType` was a name pointer plus an `i32`,
// which is 16.
const _: () = assert!(core::mem::size_of::<SensorType>() == 1);
