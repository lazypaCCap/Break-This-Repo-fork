...
//! `minecraft:enchantment_provider_type`, as a closed Rust enum.

use crate::types::RegistryId;

/// An entry of `minecraft:enchantment_provider_type` (3 of them).
///
...
/// is an id arriving off the wire, and [`EnchantmentProviderType::from_id`] is the only door for
/// it.
///
/// The discriminant is the network id, so [`EnchantmentProviderType::id`] is an `as` cast: a
/// known variant's id folds to a constant at compile time.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, PartialOrd, Ord)]
#[repr(u8)]
pub enum EnchantmentProviderType {
    /// `minecraft:by_cost` (id 0).
    ByCost = 0,
    /// `minecraft:by_cost_with_difficulty` (id 1).
    ByCostWithDifficulty = 1,
    /// `minecraft:single` (id 2).
    Single = 2,
}

/// Entry names in network-id order, so the index is the id.
///
/// A table and not a `match`, so [`EnchantmentProviderType::name`] is one load from a static.
/// `crate::generated::registry::ENCHANTMENT_PROVIDER_TYPE` points at this same array, which is
...
/// Discriminants ordered by name, so [`EnchantmentProviderType::from_name`] is a binary search
/// and not a scan.
///
...
impl EnchantmentProviderType {
    ...
    /// The namespaced name, e.g. `minecraft:by_cost`.
    ///
    ...
    ///
    /// The decode boundary, and the only place a `EnchantmentProviderType` can fail to exist.
...
impl core::fmt::Display for EnchantmentProviderType {
...
const _: () = assert!(EnchantmentProviderType::ALL[2].id().0 == 2);
...
// which is 16.
const _: () = assert!(core::mem::size_of::<EnchantmentProviderType>() == 1);
