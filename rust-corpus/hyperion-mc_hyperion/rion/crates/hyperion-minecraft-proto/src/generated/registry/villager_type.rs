...
//! `minecraft:villager_type`, as a closed Rust enum.

use crate::types::RegistryId;

/// An entry of `minecraft:villager_type` (7 of them).
///
...
/// is an id arriving off the wire, and [`VillagerType::from_id`] is the only door for
/// it.
///
/// The discriminant is the network id, so [`VillagerType::id`] is an `as` cast: a
/// known variant's id folds to a constant at compile time.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, PartialOrd, Ord)]
#[repr(u8)]
pub enum VillagerType {
    /// `minecraft:desert` (id 0).
    Desert = 0,
    /// `minecraft:jungle` (id 1).
    Jungle = 1,
    /// `minecraft:plains` (id 2).
    Plains = 2,
    /// `minecraft:savanna` (id 3).
    Savanna = 3,
    /// `minecraft:snow` (id 4).
    Snow = 4,
    /// `minecraft:swamp` (id 5).
    Swamp = 5,
    /// `minecraft:taiga` (id 6).
    Taiga = 6,
}

/// Entry names in network-id order, so the index is the id.
///
/// A table and not a `match`, so [`VillagerType::name`] is one load from a static.
/// `crate::generated::registry::VILLAGER_TYPE` points at this same array, which is
...
/// Discriminants ordered by name, so [`VillagerType::from_name`] is a binary search
/// and not a scan.
///
...
impl VillagerType {
    ...
    pub const REGISTRY: &'static str = "minecraft:villager_type";
    ...
    /// The namespaced name, e.g. `minecraft:desert`.
    ///
    ...
    ///
    /// The decode boundary, and the only place a `VillagerType` can fail to exist.
...
impl core::fmt::Display for VillagerType {
...
const _: () = assert!(VillagerType::ALL[6].id().0 == 6);
...
// which is 16.
const _: () = assert!(core::mem::size_of::<VillagerType>() == 1);
