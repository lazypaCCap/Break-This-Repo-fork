...
//! `minecraft:worldgen/feature_size_type`, as a closed Rust enum.

use crate::types::RegistryId;

/// An entry of `minecraft:worldgen/feature_size_type` (2 of them).
///
...
/// is an id arriving off the wire, and [`WorldgenFeatureSizeType::from_id`] is the only door for
/// it.
///
/// The discriminant is the network id, so [`WorldgenFeatureSizeType::id`] is an `as` cast: a
/// known variant's id folds to a constant at compile time.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, PartialOrd, Ord)]
#[repr(u8)]
pub enum WorldgenFeatureSizeType {
    /// `minecraft:two_layers_feature_size` (id 0).
    TwoLayersFeatureSize = 0,
    /// `minecraft:three_layers_feature_size` (id 1).
    ThreeLayersFeatureSize = 1,
}

/// Entry names in network-id order, so the index is the id.
///
/// A table and not a `match`, so [`WorldgenFeatureSizeType::name`] is one load from a static.
/// `crate::generated::registry::WORLDGEN_FEATURE_SIZE_TYPE` points at this same array, which is
/// what stops the two from disagreeing.
pub static NAMES_IN_ID_ORDER: [&str; 2] = [
    "minecraft:two_layers_feature_size",
    "minecraft:three_layers_feature_size",
];

/// Discriminants ordered by name, so [`WorldgenFeatureSizeType::from_name`] is a binary search
/// and not a scan.
///
...
impl WorldgenFeatureSizeType {
    ...
    pub const ALL: &'static [Self] = &[Self::TwoLayersFeatureSize, Self::ThreeLayersFeatureSize];
    /// How many entries this version has, so every valid id is below it.
    pub const COUNT: usize = 2;
    /// The registry these values belong to.
    pub const REGISTRY: &'static str = "minecraft:worldgen/feature_size_type";
    ...
    /// The namespaced name, e.g. `minecraft:two_layers_feature_size`.
    ///
    ...
    ///
    /// The decode boundary, and the only place a `WorldgenFeatureSizeType` can fail to exist.
...
impl core::fmt::Display for WorldgenFeatureSizeType {
...
const _: () = assert!(WorldgenFeatureSizeType::ALL[1].id().0 == 1);
...
// which is 16.
const _: () = assert!(core::mem::size_of::<WorldgenFeatureSizeType>() == 1);
