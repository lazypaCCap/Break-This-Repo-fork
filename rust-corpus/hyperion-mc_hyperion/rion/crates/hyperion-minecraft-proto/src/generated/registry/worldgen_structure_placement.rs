...
//! `minecraft:worldgen/structure_placement`, as a closed Rust enum.

use crate::types::RegistryId;

/// An entry of `minecraft:worldgen/structure_placement` (2 of them).
///
...
/// is an id arriving off the wire, and [`WorldgenStructurePlacement::from_id`] is the only door for
/// it.
///
/// The discriminant is the network id, so [`WorldgenStructurePlacement::id`] is an `as` cast: a
/// known variant's id folds to a constant at compile time.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, PartialOrd, Ord)]
#[repr(u8)]
pub enum WorldgenStructurePlacement {
    /// `minecraft:random_spread` (id 0).
    RandomSpread = 0,
    /// `minecraft:concentric_rings` (id 1).
    ConcentricRings = 1,
}

/// Entry names in network-id order, so the index is the id.
///
/// A table and not a `match`, so [`WorldgenStructurePlacement::name`] is one load from a static.
/// `crate::generated::registry::WORLDGEN_STRUCTURE_PLACEMENT` points at this same array, which is
/// what stops the two from disagreeing.
pub static NAMES_IN_ID_ORDER: [&str; 2] = ["minecraft:random_spread", "minecraft:concentric_rings"];

/// Discriminants ordered by name, so [`WorldgenStructurePlacement::from_name`] is a binary search
/// and not a scan.
///
...
impl WorldgenStructurePlacement {
    ...
    /// The namespaced name, e.g. `minecraft:random_spread`.
    ///
    ...
    ///
    /// The decode boundary, and the only place a `WorldgenStructurePlacement` can fail to exist.
...
impl core::fmt::Display for WorldgenStructurePlacement {
...
const _: () = assert!(WorldgenStructurePlacement::ALL[1].id().0 == 1);
...
// which is 16.
const _: () = assert!(core::mem::size_of::<WorldgenStructurePlacement>() == 1);
