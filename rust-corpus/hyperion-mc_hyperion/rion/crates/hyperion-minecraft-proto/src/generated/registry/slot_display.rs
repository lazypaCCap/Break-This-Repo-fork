...
//! `minecraft:slot_display`, as a closed Rust enum.

use crate::types::RegistryId;

/// An entry of `minecraft:slot_display` (11 of them).
///
...
/// is an id arriving off the wire, and [`SlotDisplay::from_id`] is the only door for
/// it.
///
/// The discriminant is the network id, so [`SlotDisplay::id`] is an `as` cast: a
...
pub enum SlotDisplay {
    ...
    Empty = 0,
    /// `minecraft:any_fuel` (id 1).
    AnyFuel = 1,
    /// `minecraft:with_any_potion` (id 2).
    WithAnyPotion = 2,
    /// `minecraft:only_with_component` (id 3).
    OnlyWithComponent = 3,
    /// `minecraft:item` (id 4).
    Item = 4,
    /// `minecraft:item_stack` (id 5).
    ItemStack = 5,
    /// `minecraft:tag` (id 6).
    Tag = 6,
    /// `minecraft:dyed` (id 7).
    Dyed = 7,
    /// `minecraft:smithing_trim` (id 8).
    SmithingTrim = 8,
    /// `minecraft:with_remainder` (id 9).
    WithRemainder = 9,
    /// `minecraft:composite` (id 10).
    Composite = 10,
}

/// Entry names in network-id order, so the index is the id.
///
/// A table and not a `match`, so [`SlotDisplay::name`] is one load from a static.
/// `crate::generated::registry::SLOT_DISPLAY` points at this same array, which is
...
/// Discriminants ordered by name, so [`SlotDisplay::from_name`] is a binary search
/// and not a scan.
///
...
static BY_NAME: [u8; 11] = [1, 10, 7, 0, 4, 5, 3, 8, 6, 2, 9];
...
impl SlotDisplay {
    ...
    pub const REGISTRY: &'static str = "minecraft:slot_display";
    ...
    ///
    /// The decode boundary, and the only place a `SlotDisplay` can fail to exist.
...
impl core::fmt::Display for SlotDisplay {
...
const _: () = assert!(SlotDisplay::ALL[10].id().0 == 10);
...
// which is 16.
const _: () = assert!(core::mem::size_of::<SlotDisplay>() == 1);
