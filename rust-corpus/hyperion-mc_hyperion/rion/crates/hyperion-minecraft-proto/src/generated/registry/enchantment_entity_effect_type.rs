...
//! `minecraft:enchantment_entity_effect_type`, as a closed Rust enum.

use crate::types::RegistryId;

/// An entry of `minecraft:enchantment_entity_effect_type` (15 of them).
///
...
/// is an id arriving off the wire, and [`EnchantmentEntityEffectType::from_id`] is the only door for
/// it.
///
/// The discriminant is the network id, so [`EnchantmentEntityEffectType::id`] is an `as` cast: a
...
pub enum EnchantmentEntityEffectType {
    ...
    ApplyMobEffect = 1,
    /// `minecraft:change_item_damage` (id 2).
    ChangeItemDamage = 2,
    /// `minecraft:damage_entity` (id 3).
    DamageEntity = 3,
    /// `minecraft:explode` (id 4).
    Explode = 4,
    /// `minecraft:ignite` (id 5).
    Ignite = 5,
    /// `minecraft:apply_impulse` (id 6).
    ApplyImpulse = 6,
    /// `minecraft:apply_exhaustion` (id 7).
    ApplyExhaustion = 7,
    /// `minecraft:play_sound` (id 8).
    PlaySound = 8,
    /// `minecraft:replace_block` (id 9).
    ReplaceBlock = 9,
    /// `minecraft:replace_disk` (id 10).
    ReplaceDisk = 10,
    /// `minecraft:run_function` (id 11).
    RunFunction = 11,
    /// `minecraft:set_block_properties` (id 12).
    SetBlockProperties = 12,
    /// `minecraft:spawn_particles` (id 13).
    SpawnParticles = 13,
    /// `minecraft:summon_entity` (id 14).
    SummonEntity = 14,
}

/// Entry names in network-id order, so the index is the id.
///
/// A table and not a `match`, so [`EnchantmentEntityEffectType::name`] is one load from a static.
/// `crate::generated::registry::ENCHANTMENT_ENTITY_EFFECT_TYPE` points at this same array, which is
...
/// Discriminants ordered by name, so [`EnchantmentEntityEffectType::from_name`] is a binary search
/// and not a scan.
///
...
static BY_NAME: [u8; 15] = [0, 7, 6, 1, 2, 3, 4, 5, 8, 9, 10, 11, 12, 13, 14];
...
impl EnchantmentEntityEffectType {
    ...
    pub const REGISTRY: &'static str = "minecraft:enchantment_entity_effect_type";
    ...
    ///
    /// The decode boundary, and the only place a `EnchantmentEntityEffectType` can fail to exist.
...
impl core::fmt::Display for EnchantmentEntityEffectType {
...
const _: () = assert!(EnchantmentEntityEffectType::ALL[14].id().0 == 14);
...
// which is 16.
const _: () = assert!(core::mem::size_of::<EnchantmentEntityEffectType>() == 1);
