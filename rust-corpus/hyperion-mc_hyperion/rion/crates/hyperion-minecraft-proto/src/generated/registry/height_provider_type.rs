...
//! `minecraft:height_provider_type`, as a closed Rust enum.

use crate::types::RegistryId;

/// An entry of `minecraft:height_provider_type` (6 of them).
///
...
/// is an id arriving off the wire, and [`HeightProviderType::from_id`] is the only door for
/// it.
///
/// The discriminant is the network id, so [`HeightProviderType::id`] is an `as` cast: a
...
pub enum HeightProviderType {
    ...
    BiasedToBottom = 2,
    /// `minecraft:very_biased_to_bottom` (id 3).
    VeryBiasedToBottom = 3,
    /// `minecraft:trapezoid` (id 4).
    Trapezoid = 4,
    /// `minecraft:weighted_list` (id 5).
    WeightedList = 5,
}

/// Entry names in network-id order, so the index is the id.
///
/// A table and not a `match`, so [`HeightProviderType::name`] is one load from a static.
/// `crate::generated::registry::HEIGHT_PROVIDER_TYPE` points at this same array, which is
...
/// Discriminants ordered by name, so [`HeightProviderType::from_name`] is a binary search
/// and not a scan.
///
...
static BY_NAME: [u8; 6] = [2, 0, 4, 1, 3, 5];
...
impl HeightProviderType {
    ...
    pub const REGISTRY: &'static str = "minecraft:height_provider_type";
    ...
    ///
    /// The decode boundary, and the only place a `HeightProviderType` can fail to exist.
...
impl core::fmt::Display for HeightProviderType {
...
const _: () = assert!(HeightProviderType::ALL[5].id().0 == 5);
...
// which is 16.
const _: () = assert!(core::mem::size_of::<HeightProviderType>() == 1);
