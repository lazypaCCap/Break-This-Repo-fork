...
//! `minecraft:dialog_type`, as a closed Rust enum.

use crate::types::RegistryId;

/// An entry of `minecraft:dialog_type` (5 of them).
///
...
/// is an id arriving off the wire, and [`DialogType::from_id`] is the only door for
/// it.
///
/// The discriminant is the network id, so [`DialogType::id`] is an `as` cast: a
/// known variant's id folds to a constant at compile time.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, PartialOrd, Ord)]
#[repr(u8)]
pub enum DialogType {
    /// `minecraft:notice` (id 0).
    Notice = 0,
    /// `minecraft:server_links` (id 1).
    ServerLinks = 1,
    /// `minecraft:dialog_list` (id 2).
    DialogList = 2,
    /// `minecraft:multi_action` (id 3).
    MultiAction = 3,
    /// `minecraft:confirmation` (id 4).
    Confirmation = 4,
}

/// Entry names in network-id order, so the index is the id.
///
/// A table and not a `match`, so [`DialogType::name`] is one load from a static.
/// `crate::generated::registry::DIALOG_TYPE` points at this same array, which is
...
/// Discriminants ordered by name, so [`DialogType::from_name`] is a binary search
/// and not a scan.
///
...
impl DialogType {
    ...
    pub const REGISTRY: &'static str = "minecraft:dialog_type";
    ...
    /// The namespaced name, e.g. `minecraft:notice`.
    ///
    ...
    ///
    /// The decode boundary, and the only place a `DialogType` can fail to exist.
...
impl core::fmt::Display for DialogType {
...
const _: () = assert!(DialogType::ALL[4].id().0 == 4);
...
// which is 16.
const _: () = assert!(core::mem::size_of::<DialogType>() == 1);
