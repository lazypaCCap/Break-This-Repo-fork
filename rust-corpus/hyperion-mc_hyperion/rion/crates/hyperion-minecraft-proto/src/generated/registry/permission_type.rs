...
//! `minecraft:permission_type`, as a closed Rust enum.

use crate::types::RegistryId;

/// An entry of `minecraft:permission_type` (2 of them).
///
...
/// is an id arriving off the wire, and [`PermissionType::from_id`] is the only door for
/// it.
///
/// The discriminant is the network id, so [`PermissionType::id`] is an `as` cast: a
...
pub enum PermissionType {
    /// `minecraft:atom` (id 0).
    Atom = 0,
    /// `minecraft:command_level` (id 1).
    CommandLevel = 1,
}

/// Entry names in network-id order, so the index is the id.
///
/// A table and not a `match`, so [`PermissionType::name`] is one load from a static.
/// `crate::generated::registry::PERMISSION_TYPE` points at this same array, which is
/// what stops the two from disagreeing.
pub static NAMES_IN_ID_ORDER: [&str; 2] = ["minecraft:atom", "minecraft:command_level"];

/// Discriminants ordered by name, so [`PermissionType::from_name`] is a binary search
/// and not a scan.
///
...
impl PermissionType {
    ...
    pub const ALL: &'static [Self] = &[Self::Atom, Self::CommandLevel];
    /// How many entries this version has, so every valid id is below it.
    pub const COUNT: usize = 2;
    /// The registry these values belong to.
    pub const REGISTRY: &'static str = "minecraft:permission_type";
    ...
    /// The namespaced name, e.g. `minecraft:atom`.
    ///
    ...
    ///
    /// The decode boundary, and the only place a `PermissionType` can fail to exist.
...
const _: () = assert!(PermissionType::ALL[1].id().0 == 1);
...
// which is 16.
const _: () = assert!(core::mem::size_of::<PermissionType>() == 1);
