...
//! `minecraft:worldgen/structure_pool_element`, as a closed Rust enum.

use crate::types::RegistryId;

/// An entry of `minecraft:worldgen/structure_pool_element` (5 of them).
///
...
/// is an id arriving off the wire, and [`WorldgenStructurePoolElement::from_id`] is the only door for
/// it.
///
/// The discriminant is the network id, so [`WorldgenStructurePoolElement::id`] is an `as` cast: a
/// known variant's id folds to a constant at compile time.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, PartialOrd, Ord)]
#[repr(u8)]
pub enum WorldgenStructurePoolElement {
    /// `minecraft:single_pool_element` (id 0).
    SinglePoolElement = 0,
    /// `minecraft:list_pool_element` (id 1).
    ListPoolElement = 1,
    /// `minecraft:feature_pool_element` (id 2).
    FeaturePoolElement = 2,
    /// `minecraft:empty_pool_element` (id 3).
    EmptyPoolElement = 3,
    /// `minecraft:legacy_single_pool_element` (id 4).
    LegacySinglePoolElement = 4,
}

/// Entry names in network-id order, so the index is the id.
///
/// A table and not a `match`, so [`WorldgenStructurePoolElement::name`] is one load from a static.
/// `crate::generated::registry::WORLDGEN_STRUCTURE_POOL_ELEMENT` points at this same array, which is
...
/// Discriminants ordered by name, so [`WorldgenStructurePoolElement::from_name`] is a binary search
/// and not a scan.
///
...
static BY_NAME: [u8; 5] = [3, 2, 4, 1, 0];

impl WorldgenStructurePoolElement {
    /// Every entry, in network-id order, so the index is the id.
    pub const ALL: &'static [Self] = &[
        Self::SinglePoolElement,
        Self::ListPoolElement,
        Self::FeaturePoolElement,
        Self::EmptyPoolElement,
        Self::LegacySinglePoolElement,
    ];
    /// How many entries this version has, so every valid id is below it.
    pub const COUNT: usize = 5;
    /// The registry these values belong to.
    pub const REGISTRY: &'static str = "minecraft:worldgen/structure_pool_element";
    ...
    /// The namespaced name, e.g. `minecraft:single_pool_element`.
    ///
    ...
    ///
    /// The decode boundary, and the only place a `WorldgenStructurePoolElement` can fail to exist.
...
impl core::fmt::Display for WorldgenStructurePoolElement {
...
const _: () = assert!(WorldgenStructurePoolElement::ALL[4].id().0 == 4);
...
// which is 16.
const _: () = assert!(core::mem::size_of::<WorldgenStructurePoolElement>() == 1);
