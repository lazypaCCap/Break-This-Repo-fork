...
//! `minecraft:chunk_status`, as a closed Rust enum.

use crate::types::RegistryId;

/// An entry of `minecraft:chunk_status` (12 of them).
///
...
/// is an id arriving off the wire, and [`ChunkStatus::from_id`] is the only door for
/// it.
///
/// The discriminant is the network id, so [`ChunkStatus::id`] is an `as` cast: a
...
pub enum ChunkStatus {
    ...
    Empty = 0,
    /// `minecraft:structure_starts` (id 1).
    StructureStarts = 1,
    /// `minecraft:structure_references` (id 2).
    StructureReferences = 2,
    /// `minecraft:biomes` (id 3).
    Biomes = 3,
    /// `minecraft:noise` (id 4).
    Noise = 4,
    /// `minecraft:surface` (id 5).
    Surface = 5,
    /// `minecraft:carvers` (id 6).
    Carvers = 6,
    /// `minecraft:features` (id 7).
    Features = 7,
    /// `minecraft:initialize_light` (id 8).
    InitializeLight = 8,
    /// `minecraft:light` (id 9).
    Light = 9,
    /// `minecraft:spawn` (id 10).
    Spawn = 10,
    /// `minecraft:full` (id 11).
    Full = 11,
}

/// Entry names in network-id order, so the index is the id.
///
/// A table and not a `match`, so [`ChunkStatus::name`] is one load from a static.
/// `crate::generated::registry::CHUNK_STATUS` points at this same array, which is
/// what stops the two from disagreeing.
pub static NAMES_IN_ID_ORDER: [&str; 12] = [
    "minecraft:empty",
...
/// Discriminants ordered by name, so [`ChunkStatus::from_name`] is a binary search
/// and not a scan.
///
...
/// 12 names back through `from_name` rather than spot-checking.
static BY_NAME: [u8; 12] = [3, 6, 0, 7, 11, 8, 9, 4, 10, 2, 1, 5];
...
impl ChunkStatus {
    ...
    pub const REGISTRY: &'static str = "minecraft:chunk_status";
    ...
    ///
    /// The decode boundary, and the only place a `ChunkStatus` can fail to exist.
...
impl core::fmt::Display for ChunkStatus {
...
const _: () = assert!(ChunkStatus::ALL[11].id().0 == 11);
...
// which is 16.
const _: () = assert!(core::mem::size_of::<ChunkStatus>() == 1);
