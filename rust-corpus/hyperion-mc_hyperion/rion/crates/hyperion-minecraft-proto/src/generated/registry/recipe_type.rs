...
//! `minecraft:recipe_type`, as a closed Rust enum.

use crate::types::RegistryId;

/// An entry of `minecraft:recipe_type` (7 of them).
///
...
/// is an id arriving off the wire, and [`RecipeType::from_id`] is the only door for
/// it.
///
/// The discriminant is the network id, so [`RecipeType::id`] is an `as` cast: a
/// known variant's id folds to a constant at compile time.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, PartialOrd, Ord)]
#[repr(u8)]
pub enum RecipeType {
    /// `minecraft:crafting` (id 0).
    Crafting = 0,
    /// `minecraft:smelting` (id 1).
    Smelting = 1,
    /// `minecraft:blasting` (id 2).
    Blasting = 2,
    /// `minecraft:smoking` (id 3).
    Smoking = 3,
    /// `minecraft:campfire_cooking` (id 4).
    CampfireCooking = 4,
    /// `minecraft:stonecutting` (id 5).
    Stonecutting = 5,
    /// `minecraft:smithing` (id 6).
    Smithing = 6,
}

/// Entry names in network-id order, so the index is the id.
///
/// A table and not a `match`, so [`RecipeType::name`] is one load from a static.
/// `crate::generated::registry::RECIPE_TYPE` points at this same array, which is
...
/// Discriminants ordered by name, so [`RecipeType::from_name`] is a binary search
/// and not a scan.
///
...
static BY_NAME: [u8; 7] = [2, 4, 0, 1, 6, 3, 5];
...
impl RecipeType {
    ...
    pub const REGISTRY: &'static str = "minecraft:recipe_type";
    ...
    /// The namespaced name, e.g. `minecraft:crafting`.
    ///
    ...
    ///
    /// The decode boundary, and the only place a `RecipeType` can fail to exist.
...
impl core::fmt::Display for RecipeType {
...
const _: () = assert!(RecipeType::ALL[6].id().0 == 6);
...
// which is 16.
const _: () = assert!(core::mem::size_of::<RecipeType>() == 1);
